...
//print("The value of foo is \(foo)")
//because foo is defined inside the scope, hence it cannot be accessed outside
...
//x is defined both in the gloabl scope and the block scope so when x values are orinted two times in different scopes we can see 2 diff values of x
//it is diff from foo because foo was declared inside the block scope which can't be accessed by the gloabl scope.
